...
@patch("crystallography_bluesky.i15_1.plans.robot.robot_unload")
async def test_given_something_already_loaded_then_robot_will_unload(
...
@patch("crystallography_bluesky.i15_1.plans.robot.robot_unload")
async def test_given_unload_fails_then_load_does_not_happen(
    mock_unload: MagicMock,
    robot: Robot,
    hutch_interlock: PSSInterlock,
    gonio_interlock: IntPLCInterlock,
    ...
    set_mock_value(robot.current_sample.puck, 8)
    set_mock_value(robot.current_sample.position, 1)
    mock_unload.side_effect = ValueError()
